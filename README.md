...
### 1. Build the frontend (not needed if u downloaded latest release)
...
Which export you want depends on **where your code runs**:
 
| Your script | Export | Goes to |
| --- | --- | --- |
| client | `CustomAlert` | every on-duty player matching `jobs` |
| **server** | **`ServerAlert`** | every on-duty player matching `jobs`, or only `targets` |
| server | `SendTargetedAlert` | only the players you list (low-level, no defaults) |
...
`ServerAlert` is the entry point for anything that happens on the server: a shop
robbery starting, a bank timer expiring, an MDT warrant going live, a webhook
coming in. It runs the exact same pipeline as a client alert — merging,
escalation, hotspots, statistics, job filtering — so a server-raised call
behaves identically to one reported by a player.
 
### Alert every matching officer
...
exports['ps-dispatch']:ServerAlert({
    ...
    icon = 'fas fa-store',
    priority = 1,
    coords = vector3(1135.0, -982.0, 46.0),
    jobs = { 'leo' },
})
```
 
### Alert one specific player (or a few)
 
Add `targets` — a server id, or a table of them. Everything else stays the same:
...
exports['ps-dispatch']:ServerAlert({
    ...
    information = 'Meet the complainant at the front desk',
    targets = src,             -- or { srcA, srcB }
    addToList = true,          -- optional: also put it on the shared board
})
```
 
### Fields
 
Only `message` and `coords` are required. `coords` takes a `vector3` or a plain
`{ x =, y =, z = }` table.
...
| `street` | resolved automatically, see below |
| `targets` | none — broadcast to `jobs` |
| `addToList` | `false` for targeted alerts; broadcasts are always listed |
 
Every field from the [Alert payload](#alert-payload) table works here too —
`vehicle`, `plate`, `weapon`, `information`, `footer` and the rest.
 
Returns `true` when the payload was accepted, `false` when it was rejected
(missing `message`, or `coords` without usable `x`/`y`).
 
### Getting the call id
 
The return value only tells you the payload passed validation — the call itself
may be created a moment later while the street is resolved. Pass a callback as
the second argument to get the id and the final payload:
 
```lua
exports['ps-dispatch']:ServerAlert(data, function(id, sent)
    print(('call #%s at %s'):format(id, sent.street or 'unknown'))
end)
```
 
`id` is `nil` if the alert was rejected downstream; for a broadcast that merged
into an existing call, it's the id of that call.
 
### About `street`
 
`GetStreetAndZone` is a client native with no server equivalent, so the server
cannot name a location on its own — and a call without a street loses its
location line in the UI *and* drops out of hotspot tracking, which keys on
street names.
 
`ServerAlert` solves this by asking the player **closest to the alert** to
resolve the coordinates. The nearest one is picked deliberately: the zone half
of the label works from anywhere, but the street name is read from
region-streamed path nodes, so a player on the other side of the map would
usually come back with the zone alone.
 
Costs one round trip, capped at 1.5 s, and only for alerts that don't already
carry a `street`. Pass `street = 'Alta Street'` yourself to skip it entirely, or
set `Config.ResolveStreet = false` to turn it off globally and leave the field
empty. If nobody is close enough, the alert simply carries the zone (`Del
Perro`) instead of the full label (`Vespucci Boulevard, Del Perro`).
 
### Blips for custom code names
 
A `codeName` with no `Config.Blips` entry gets a neutral default marker so the
alert still appears on the map. Add an entry under the same `codeName` for a
proper sprite, colour, radius, sound and position offset.
 
### SendTargetedAlert (low-level)
 
`ServerAlert` with `targets` covers almost every case. `SendTargetedAlert` is
the raw form underneath it: no defaults, no validation beyond `message`, no
street resolution — any field you pass arrives untouched. Useful when you want
full control over the payload, as `ps-dispatch` itself does for plate checks:
...
gender. From a server script use [`ServerAlert`](#server-side-alerts) instead.
